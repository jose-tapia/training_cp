...
set<int> freqs_existentes;
map<int, int> bucket;
map<int, int> freqs;
vector<pii> obtener_ocurrencias() {
    vector<pii> ocurrencias;
    // Ya estaran en orden porque estamos recorriendo un set ordenado
    for (int num: freqs_existentes) {
        ocurrencias.push_back({num, freqs[num]});
    }
    return ocurrencias;
}
void actualizar_bucket(int val, int cambio) {
    if (freqs[bucket[val]] == 1) {
        freqs_existentes.erase(bucket[val]);
    }
    freqs[bucket[val]] -= 1;
    bucket[val] += cambio;
    freqs[bucket[val]] += 1;
    if (freqs[bucket[val]] == 1) {
        freqs_existentes.insert(bucket[val]);
    }
}
